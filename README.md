...
`touchdesigner/exports/regions/regions.json`, generated by `tools/split_lobes.py`.
...
It extracts bpm, beats and confidence, key, structural sections (intro/verse/chorus/bridge/
outro), descriptors and mood. **Essentia** does the signal work, **librosa** only the
structural segmentation, **TensorFlow** the mood.
...
| Script | Purpose |
|---|---|
| `tools/split_lobes.py` | from the segmented FBX model to the 7 per-region OBJs |
| `tools/make_wireframe.py` | from the 7 OBJs to the panel's anatomical drawing |
...
| `tools/compare_tracks.py` | compares the mapping across the tracks in the library |
| `tools/preview_regions.py` | control PNG of the segmentation |
| `tools/bench_face.py` | measures the cost of the two MediaPipe trackers |
